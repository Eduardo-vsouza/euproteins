...
class TranscriptAssembly(object):
    ...
    def merge_transcripts(self, tpm_cutoff=1, fpkm_cutoff=1):
        cmd_str = f'stringtie --merge StringTie/gtf_list.txt -o assembled.gtf -G {self.gtf} -F {fpkm_cutoff}'\
                  f' -T {tpm_cutoff} -m 30 -l uproteins'
        os.system(cmd_str)
...
class CompareTranscripts(object):
    def __init__(self, args, assembled_gtf):
        ...
        self.copy_genome()
        self.index_genome()
        self.dependencies = f'{sys.path[0]}/dependencies'
    ...
    def __check_path(self):
        if self.args.gff_compare_path is not None:
            ...
            gff_path = f'{self.dependencies}/gffcompare-0.12.6.Linux_x86_64/gffcompare'
        return gff_path
