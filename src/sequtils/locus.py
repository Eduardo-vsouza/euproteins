...
class RefSeqGFF(object):
    ...
    def __fix_gtf(self):
        with open(self.gff, 'r') as handler, open(f'{self.gff}_columns', 'w') as out:
            to_write = ['seqname\tsource\tfeature\tstart\tend\tscore\tstrand\tframe\tattributes\n']
    ...
    def __read_gff(self):
        print("we are here")
        df = pd.read_csv(f'{self.gff}_columns', sep='\t')
...
class StringTieGFF(object):
    ...
    def __read_gff(self):
        ...
        for i in range(len(attrs)):
            if attrs[i].startswith('transcript_id'):
                name = attrs[i].split(";")[0][15:-1]
                print('transcript id:', name)
